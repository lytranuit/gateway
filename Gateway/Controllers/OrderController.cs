...
namespace ApplicationChart.Controllers
{
    ...
    public class OrderController : MyBaseController
    {
        ...
        public ListData DATATAO(string x, string MATDV, List<string> matinh = null)
        {
            ...
            {
                var listMATDV = MATDV.Split(',').ToList();
                strcn = "SELECT makh AS MAKH, donvi AS DONVI ,hanmuc as HANMUC, matdv AS MATDV, diachi AS DIACHI, phanloaikhachhang,macn from TBL_DANHMUCKHACHHANG" + string.Format(" WHERE tk = '131' and  (tinhtrang != N'Ngừng giao dịch' or tinhtrang is null) AND (matdv IN ({0}))", string.Join(",", listMATDV.Select(p => "'" + p + "'")));
            }
            else
            {
                strcn = "SELECT makh AS MAKH, donvi AS DONVI,hanmuc as HANMUC, matdv AS MATDV, diachi AS DIACHI,phanloaikhachhang,macn from TBL_DANHMUCKHACHHANG WHERE tk = '131' and  (tinhtrang != N'Ngừng giao dịch' or tinhtrang is null) ";
            }
